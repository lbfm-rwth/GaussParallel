...
RiffleIn := function( X,u,nr,f,type )
  local i,F,new,curX,curF;
  new := [];
  ...
  for i in [ 1 .. Length(u) ] do
      if u[i] = 1 then
         Add( new,TransposedMat(X)[curX] ); curX := curX + 1;
      else
         Add( new,F[curF] ); curF := curF +1;  
      fi;
  od;
...
UpdateTrafo := function( f, T, K, M,v,flag,dbug )
 local A, E, MM, KK, s, u,  tmp, Z, V, X, W, Y,S,nr;
 A:=T[1];MM:=T[2];E:=T[3];KK:=T[4];s:=T[5];u:=T[6];
   ...
   if IsEmpty(K) then
      if not IsEmpty(M) then
         nr := DimensionsMat(M)[2]; 
      else
         return[ KK, RRF(E,MM,u) ];
      fi;
   else
      nr := DimensionsMat(K)[1];
   fi;

   if IsEmpty(M) then
      Y := RiffleIn( K,v,nr,f,flag );
      tmp := REX( f,BitstringToCharFct(s),Y );
      V := tmp[1];
      W := tmp[2];
   
      if not IsEmpty(V) then # else we have MM and s empty
         M := MM*V; 
      fi;
   else
      Y := RiffleIn( K,v,nr,f,flag ); 
...
GaussParallel := function( Inp,a,b ) #Chop inputmatrix Inp into (a)x(b) matrix
    local C,D,B,A,E,F,M,K,X,R, tmp,f,i,j,k,h,v,dbug;
    C := ChoppedMatrix( Inp,a,b );v :=[];R := [];A := [];B := [];D := [];E := [];F := [];M := [];K := [];X := [];
    ...
    for i in [ 1 .. a ] do
        for j in [ 1 .. b ] do
            if j = 1 then       
               E[i][1] := 0*[ 1 .. DimensionsMat(C[i][j])[1] ];
            fi;
            ...
            for h in [ 1 .. i-1 ] do 
                tmp := UpdateTrafo( f,A[i][j],K[i][h],M[j][h],v[h][j],0,dbug );
                K[i][h] := tmp[1];
                M[j][h] := tmp[2];
            od;
            tmp := UpdateTrafo( f,A[i][j],K[i][i],M[j][i],v[i][j],1,dbug );
            K[i][i] := tmp[1];
            M[j][i] := tmp[2];
            Error( "DBUG----");
        od;
