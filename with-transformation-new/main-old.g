...
Step1 := function( A,n )
 local S,C, cur,f, nrr, tmp,K,  l, i, j, k, B,Q,Qj, T, Rj, H, tj, V, W;
 f := DefaultFieldOfMatrix( A );
 C := ChopMatrix( n, A );
 nrr := DimensionsMat(A)[1];
 B := [];S:=[]; K:=[]; Q:=[]; #Init B as nxn; S remembers the extraction of pivotrows
 for i in [1..n] do
  B[i] :=[]; K[i]:=[];S[i]:=[]; Q[i]:=[]; 
  for j in [1..n] do
   B[i][j]:=[];
 ...
 for i in [1..n] do
  K[i][i] := IdentityMat( DimensionsMat(C[i][i])[1],f  );
  ...
  for j in [1..n] do
   ...
   if j=1 then
       S[i][j] := ExtendPivotRows(S[i][1],T[5]);
   else
       S[i][j] := ExtendPivotRows(S[i][j-1],T[5]);
   fi;
   ...
   for l in [1..i] do 
    tmp := UpdateTrafo( f, T, K[i][l], Q[j][l] );
    K[i][l] := tmp[1];
    Q[j][l] := tmp[2];
    #Error( "Break Point - before CEX new residue" );
   od;
...
s:=[];
 ...
 MM := MutableCopyMat(NullMat( rank,nrows, f ));
 KK := MutableCopyMat(NullMat( nrows-rank,ncols,f ));
 ...
 ## Build M --Zero cols of MM should later be the same as idmatrix cols of KK
 ct :=1; rct:=1;
 for i in [1..n] do
    ct := 1; tmp := 0;
    for k in [ 1 .. n ] do
        ...
        MM{[rct..DimensionsMat(M[i][k])[1]+rct-1]}{[ct..DimensionsMat(M[i][k])[2]+ct-1]} := M[i][k];
        ct := ct + DimensionsMat(M[i][k])[2];
        tmp := DimensionsMat(M[i][k])[1];
    od;
    rct := rct + tmp;
 od;
